...
nedge-topology/   ├── pyproject.toml           # Build System   ├── requirements.txt         # Dependencies (includes Streamlit/Plotly)   ├── README.md                # Documentation   ├── dashboard.py             # \[UPDATED] God-Mode with Tension Visualization   ├── proofs/                  # Agda Formal Verification   │   ├── RotationalTransport.agda   │   ├── TopologicalGating.agda   │   ├── TopologicalSuperposition.agda   │   ├── StasheffCoherence.agda   │   ├── NonAbelianTransport.agda   │   └── AdjointTransport.agda   └── nedge\_topology/          # The Python Package       ├── \_\_init\_\_.py          # API Exports       ├── geometry.py          # \[UPDATED] The Adjoint Engine (SymNum + RoPE)       ├── graph.py             # \[UPDATED] Nodes with Tension State       ├── parser.py            # \[UPDATED] Dynamic Gating based on Tension       ├── search.py            # Semantic Search Functor       ├── storage.py           # HDF5 Persistence       ├── visualizer.py        # Graphviz Exporter       ├── induction.py         # Grammar Repair       └── train.py             # Reflexive Trainer
...
st.set\_page\_config(page\_title="Nedge God-Mode", layout="wide", page\_icon="🌀")   st.title("🌀 Nedge-Topology: God-Mode Dashboard")
...
tab1, tab2 = st.tabs(\["🧠 Adjoint Manifold", "🔬 Forensic Parser"])
