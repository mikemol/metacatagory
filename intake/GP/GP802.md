...
The Volume of the Stasheff Polytope (\$K_n\$) is equal to the Semantic Discrepancy (Curvature) between the paths that form its skeleton.
...
Imagine the **Stasheff Pentagon** (\$K_4\$):
...
#### **Update nedge_topology/mitosis.py**

We add a method to calculate this \"Surface Area\" using the **Wedge Product** of the Yoneda vectors.

> Python

def calculate_holonomy(self, node_a: PackedNode, node_b: PackedNode):\ \"\"\"\ Calculates the curvature (semantic difference) between two paths\ that claim to be the same node (Ambiguity).\ \ This corresponds to the Area of the Surface between the paths.\ \"\"\"\ \# Vector A (Path 1)\ va = F.normalize(node_a.content, p=2, dim=0)\ \ \# Vector B (Path 2)\ vb = F.normalize(node_b.content, p=2, dim=0)\ \ \# 1. The Dot Product (Scalar) measures alignment (0 to 1)\ dot = torch.dot(va, vb)\ \ \# 2. The Wedge Product (Bivector) measures Area (0 to 1)\ \# In high dimensions, this is \|\|v_a \^ v_b\|\| = sin(theta)\ \# It represents the \'Volume of Ambiguity\'\ wedge_magnitude = torch.sqrt(1.0 - dot\*\*2)\ \ return wedge_magnitude.item()
