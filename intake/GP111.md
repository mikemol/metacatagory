...
nedge-topology/   ├── pyproject.toml           # Build System   ├── requirements.txt         # Updated Dependencies   ├── README.md                # Documentation   ├── dashboard.py             # \[NEW] God-Mode Control Plane (Streamlit)   ├── proofs/                  # Agda Formal Verification   │   ├── RotationalTransport.agda   │   ├── TopologicalGating.agda   │   └── TopologicalSuperposition.agda   └── nedge\_topology/          # The Python Package       ├── \_\_init\_\_.py          # \[UPDATED] API Exports       ├── geometry.py          # RoPE Group Action logic       ├── graph.py             # SPPFNode & PackedNode       ├── parser.py            # Vectorized Earley with Semantic Gating       ├── search.py            # Semantic Search Functor       ├── storage.py           # HDF5 Persistence       ├── visualizer.py        # Graphviz Exporter       ├── induction.py         # \[NEW] Grammar Repair / Rule Synthesis       └── train.py             # \[NEW] Reflexive Hebbian Training Loop
...
tab1, tab2 = st.tabs(\["🧠 Manifold", "🔬 Forensic Parser"])
