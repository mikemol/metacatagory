...
1. Load `.github/roadmap/tasks.json` to know active/planned/deferred tasks.
1. Check `DEFERRED-TRACKING.md` for current technical debt structure (postulates, TODO counts, rationale philosophy).
1. Consult `docs/ARCHITECTURE.md` for layer mapping and file purpose before editing any core module.
1. For domain ingestion tasks, reference `ingest.md` (curriculum outline) and map section → record(s)/adapter(s) needed.
1. When working inside tests, open the relevant file in `src/agda/Tests/` and confirm phase numbering alignment.
...
1. Locate related source files listed in the `files` array.
1. Add smallest cohesive Agda module changes; avoid speculative refactors.
1. Provide test scaffolding (new phase block at end of existing test suite or a dedicated checklist).
1. Run narrowed typecheck commands:
...
1. Remove a specific postulate only if constructive data is available (avoid partial broken proofs).
1. Add a DeviationLog comment if temporarily replacing a brittle proof with Bool scaffolding.
1. Re-run scan; celebrate delta in PR description.
...
1. Use concise bullet sections (≤6 bullets) with bold nouns.
1. Wrap file paths and commands in backticks.
1. Do not paste entire large Agda modules; reference paths.
1. For math, use inline `$` or block `$$` with KaTeX-compatible syntax (when needed).
1. If patching: apply minimal contextual diff, avoid unrelated reformatting.
...
1. Add concise DeviationLog entry: motivation + revisit conditions.
1. Link to issue via sync script.
