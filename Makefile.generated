...
MUTATIVE_TARGETS := regen-makefile build/graph_parsed_state.txt python-build python-test python-verified roadmap-merge build/canonical_enriched.json roadmap-enrich roadmap-export-json roadmap-export-md roadmap-export-enriched roadmap-export-deps roadmap-validate-json roadmap-validate-md roadmap-validate-triangle roadmap-sppf-export roadmap-all-enriched md-fix md-normalize docs-generate docs-modules docs-all docs-validate json-decompose json-decompose-prebuilt json-recompose json-recompose-light json-roundtrip-validate json-roundtrip-validate-light json-decompose-enriched json-recompose-enriched json-roundtrip-validate-enriched json-decompose-planning json-recompose-planning json-roundtrip-validate-planning build/canonical_roadmap.json intake-scan node-deps deferred-items act-list act-ci act-lint act-markdown-fix act-makefile-validate act-roadmap-sync act-deferred act-badges act-all badges priority-strategy-profiles priority-badge-weights priority-profile-json priority-refresh roadmap-index planning-index-json planning-kernel roadmap-sync roadmap-sppf build/diagrams/agda-deps-full.dot roadmap-deps-graph dependency-graph-json all debt-check validate-constructive check ci-light ci-preflight docker-rootless-status docker-build docker-build-ghcr docker-push-ghcr docker-all
READONLY_TARGETS := graph-status graph-assert-ok md-lint intake-lint makefile-validate
...
build/graph_parsed_state.txt: build/diagrams/agda-deps-full.dot
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) --compile src/agda/Examples/ExporterMakefile.agda && ./src/agda/ExporterMakefile); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "build/graph_parsed_state.txt" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
python-build: 
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (echo "python build: no-op (interpreted artifacts)"); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "python-build" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Run Python tests (includes pytest suite)
python-test: python-build
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (pytest tests/ -v); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "python-test" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Witness target: build + test contracted
python-verified: python-build python-test
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (echo "python verified: build+test complete"); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "python-verified" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
json-recompose: $(DEPS_DIR)
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (python3 scripts/json_recompose.py $(DEPS_DIR) build/dependency_graph_recomposed.json); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "json-recompose" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Recompose hierarchical JSON (prebuilt, fallback-safe)
json-recompose-light: json-decompose-prebuilt
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (python3 scripts/json_recompose.py $(DEPS_DIR) build/dependency_graph_recomposed.json); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "json-recompose-light" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
json-roundtrip-validate-light: json-recompose-light
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (python3 scripts/validate_json_roundtrip.py); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "json-roundtrip-validate-light" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Decompose canonical_enriched.json into item hierarchy
json-decompose-enriched: build/canonical_enriched.json
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (python3 scripts/json_decompose.py build/canonical_enriched.json data/enriched/ --strategy item-array); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "json-decompose-enriched" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Recompose enriched items into canonical_enriched.json
json-recompose-enriched: data/enriched/
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (python3 scripts/json_recompose.py data/enriched/ build/canonical_enriched_recomposed.json); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "json-recompose-enriched" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
json-recompose-planning: $(PLANNING_DIR)
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (python3 scripts/json_recompose.py $(PLANNING_DIR) build/planning_index_recomposed.json); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "json-recompose-planning" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
build/diagrams/agda-deps-full.dot: 
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (if [ -n "$${SKIP_GHC_BACKEND}" ]; then mkdir -p build/diagrams && echo "digraph G {}" > build/diagrams/agda-deps-full.dot; else mkdir -p build/diagrams && $(AGDA) --dependency-graph=build/diagrams/agda-deps-full.dot $(AGDA_FLAGS) src/agda/Tests/Index.agda 2>&1 | (grep -E "(Checking|Error)" || true) | awk 'NR<=20{print}'; fi); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "build/diagrams/agda-deps-full.dot" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Generate dependency graph
...
check: graph-assert-ok makefile-validate md-lint roadmap-validate-triangle docs-validate python-verified debt-check json-roundtrip-validate json-roundtrip-validate-enriched json-roundtrip-validate-planning all
	$(call require_mutate)
...
ci-light: graph-assert-ok makefile-validate md-lint docs-validate json-roundtrip-validate-light json-roundtrip-validate-enriched json-roundtrip-validate-planning
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (echo "ci-light complete"); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "ci-light" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Fast guard: graph + makefile docs
ci-preflight: graph-assert-ok makefile-validate
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (echo "ci-preflight complete"); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "ci-preflight" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Check rootless Docker daemon status
docker-rootless-status: 
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (source scripts/docker-rootless-env.sh && docker info --format 'Rootless: {{join .SecurityOptions ", "}}'); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "docker-rootless-status" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Build Docker image (metacatagory:dev)
docker-build: 
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (source scripts/docker-rootless-env.sh && docker build -t metacatagory:dev .); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "docker-build" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Build and tag image for GHCR (requires GHCR_REGISTRY and GHCR_USERNAME env vars)
docker-build-ghcr: 
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (source scripts/docker-rootless-env.sh && docker build -t $${GHCR_REGISTRY}/$${GHCR_USERNAME}/metacatagory:dev . && docker tag $${GHCR_REGISTRY}/$${GHCR_USERNAME}/metacatagory:dev $${GHCR_REGISTRY}/$${GHCR_USERNAME}/metacatagory:latest); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "docker-build-ghcr" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Push image to GHCR (requires docker login; set GHCR_REGISTRY and GHCR_USERNAME)
docker-push-ghcr: docker-build-ghcr
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (source scripts/docker-rootless-env.sh && docker push $${GHCR_REGISTRY}/$${GHCR_USERNAME}/metacatagory:dev && docker push $${GHCR_REGISTRY}/$${GHCR_USERNAME}/metacatagory:latest); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "docker-push-ghcr" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
# Build and push to GHCR (full pipeline)
docker-all: docker-build-ghcr docker-push-ghcr
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); (echo "Docker build and push to GHCR complete"); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "docker-all" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Tests/Examples/CrossDomainCompositionTests.agdai: src/agda/Tests/Examples/CrossDomainCompositionTests.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Tests/Examples/CrossDomainCompositionTests.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Tests/Examples/CrossDomainCompositionTests.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Tests/Plan/CIM/SchemaValidationGenericTests.agdai: src/agda/Tests/Plan/CIM/SchemaValidationGenericTests.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Tests/Plan/CIM/SchemaValidationGenericTests.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Tests/Plan/CIM/SchemaValidationGenericTests.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Tests/Plan/CIM/TypeCheckingGenericTests.agdai: src/agda/Tests/Plan/CIM/TypeCheckingGenericTests.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Tests/Plan/CIM/TypeCheckingGenericTests.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Tests/Plan/CIM/TypeCheckingGenericTests.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Tests/Plan/CIM/ABNFParserGenericTests.agdai: src/agda/Tests/Plan/CIM/ABNFParserGenericTests.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Tests/Plan/CIM/ABNFParserGenericTests.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Tests/Plan/CIM/ABNFParserGenericTests.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Tests/Plan/CIM/ProofTraceGenericTests.agdai: src/agda/Tests/Plan/CIM/ProofTraceGenericTests.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Tests/Plan/CIM/ProofTraceGenericTests.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Tests/Plan/CIM/ProofTraceGenericTests.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Tests/Core/PhaseCategory/DualityTests.agdai: src/agda/Tests/Core/PhaseCategory/DualityTests.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Tests/Core/PhaseCategory/DualityTests.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Tests/Core/PhaseCategory/DualityTests.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Infrastructure/Adequacy.agdai: src/agda/Infrastructure/Adequacy.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Infrastructure/Adequacy.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Infrastructure/Adequacy.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Infrastructure/Adequacy/Polymorphic.agdai: src/agda/Infrastructure/Adequacy/Polymorphic.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Infrastructure/Adequacy/Polymorphic.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Infrastructure/Adequacy/Polymorphic.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Infrastructure/Adequacy/CrossDomain.agdai: src/agda/Infrastructure/Adequacy/CrossDomain.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Infrastructure/Adequacy/CrossDomain.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Infrastructure/Adequacy/CrossDomain.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/Docs.agdai: src/agda/Examples/Makefile/Targets/Docs.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/Docs.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/Docs.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/Infra.agdai: src/agda/Examples/Makefile/Targets/Infra.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/Infra.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/Infra.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/Priority.agdai: src/agda/Examples/Makefile/Targets/Priority.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/Priority.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/Priority.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/Docker.agdai: src/agda/Examples/Makefile/Targets/Docker.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/Docker.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/Docker.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/JsonRoundtrip.agdai: src/agda/Examples/Makefile/Targets/JsonRoundtrip.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/JsonRoundtrip.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/JsonRoundtrip.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/RoadmapExports.agdai: src/agda/Examples/Makefile/Targets/RoadmapExports.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/RoadmapExports.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/RoadmapExports.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/Python.agdai: src/agda/Examples/Makefile/Targets/Python.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/Python.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/Python.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/AgdaBuild.agdai: src/agda/Examples/Makefile/Targets/AgdaBuild.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/AgdaBuild.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/AgdaBuild.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/Makefile/Targets/Composite.agdai: src/agda/Examples/Makefile/Targets/Composite.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/Makefile/Targets/Composite.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/Makefile/Targets/Composite.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Examples/CrossDomainComposition.agdai: src/agda/Examples/CrossDomainComposition.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Examples/CrossDomainComposition.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Examples/CrossDomainComposition.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/JSONTransformationExtraction.agdai: src/agda/Plan/CIM/JSONTransformationExtraction.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/JSONTransformationExtraction.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/JSONTransformationExtraction.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/JSONTransformationTesting.agdai: src/agda/Plan/CIM/JSONTransformationTesting.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/JSONTransformationTesting.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/JSONTransformationTesting.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/ProofTraceGeneric.agdai: src/agda/Plan/CIM/ProofTraceGeneric.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/ProofTraceGeneric.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/ProofTraceGeneric.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/JSONTransformationBackends.agdai: src/agda/Plan/CIM/JSONTransformationBackends.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/JSONTransformationBackends.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/JSONTransformationBackends.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/JSONTransformation.agdai: src/agda/Plan/CIM/JSONTransformation.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/JSONTransformation.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/JSONTransformation.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/SchemaValidationGeneric.agdai: src/agda/Plan/CIM/SchemaValidationGeneric.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/SchemaValidationGeneric.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/SchemaValidationGeneric.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/JSONTransformationAdequacy.agdai: src/agda/Plan/CIM/JSONTransformationAdequacy.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/JSONTransformationAdequacy.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/JSONTransformationAdequacy.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/JSONConcrete.agdai: src/agda/Plan/CIM/JSONConcrete.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/JSONConcrete.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/JSONConcrete.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/ABNFParserGeneric.agdai: src/agda/Plan/CIM/ABNFParserGeneric.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/ABNFParserGeneric.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/ABNFParserGeneric.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/JSONTransformationGeneric.agdai: src/agda/Plan/CIM/JSONTransformationGeneric.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/JSONTransformationGeneric.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/JSONTransformationGeneric.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/TypeCheckingGeneric.agdai: src/agda/Plan/CIM/TypeCheckingGeneric.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/TypeCheckingGeneric.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/TypeCheckingGeneric.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Plan/CIM/JSONTransformationContract.agdai: src/agda/Plan/CIM/JSONTransformationContract.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Plan/CIM/JSONTransformationContract.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Plan/CIM/JSONTransformationContract.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Algebra/Bridge/ParserAction.agdai: src/agda/Algebra/Bridge/ParserAction.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Algebra/Bridge/ParserAction.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Algebra/Bridge/ParserAction.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
...
src/agda/Core/PhaseCategory/Duality.agdai: src/agda/Core/PhaseCategory/Duality.agda
	$(call require_mutate)
	@mkdir -p $(PROFILE_DIR); start=$$(date +%s%N); ($(AGDA) $(AGDA_FLAGS) src/agda/Core/PhaseCategory/Duality.agda); rc=$$?; end=$$(date +%s%N); elapsed_ms=$$(( (end-start)/1000000 )); status=$$( [ $$rc -eq 0 ] && echo ok || echo fail ); printf '{"target":"%s","start_ns":%s,"end_ns":%s,"elapsed_ms":%s,"status":"%s"}\n' "src/agda/Core/PhaseCategory/Duality.agdai" $$start $$end $$elapsed_ms $$status >> $(PROFILE_LOG); exit $$rc
