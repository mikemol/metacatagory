...
PRIO-ADOPT-1: Priority Strategy Integration
- Weight profiles are semantically aligned with Agda TechnicalDebt.Priorities strategies
- Mapping: proof→postulate, documentation→todo, safety→fixme, critical→deviation
- See scripts/adopt_priority_strategies.py for the mapping logic
- BADGE_WEIGHTS_PROFILE environment variable selects which strategy to use
...
# ============================================================================
# LOGIC LAYER: Load pure weights data from adopt_priority_strategies
# ============================================================================

def load_priority_weights(output_dir: Path, profile: str = "default") -> dict:
    """
    LOGIC LAYER: Load weight profiles generated by adopt_priority_strategies.py
    
    This function uses the output of pure mapping logic (from TechnicalDebt.Priorities).
    No formatting or presentation concerns here - just compute/load domain data.
    """
    # For now, use embedded weights.json until we integrate full adopt_priority_strategies flow
    # In production, this would load from weights-agda-mapped.json or invoke adopt_priority_strategies
    ...
    if weights_file.exists():
        try:
            ...
            # Extract specified profile
            profiles = weights_data.get("profiles", {})
...
def load_weights(output_dir: Path) -> dict[str, float]:
    """
    LOGIC LAYER: Load severity weights from external JSON or use defaults.
    
    Supports profile selection via env BADGE_WEIGHTS_PROFILE. 
    Weights come from TechnicalDebt.Priorities via adopt_priority_strategies.py
    
    Accepts either:
    - Flat dict {"postulate": 2.0, ...} (legacy)
    - Profile map {"profiles": {"default": {...}, "ffiSafety": {...}}, "active": "default"}
    
    This is pure logic - no formatting side effects.
    """
