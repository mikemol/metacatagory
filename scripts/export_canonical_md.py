...
from scripts.shared.paths import (
    ...
    PLANNING_INDEX_JSON as _DEFAULT_PLANNING_INDEX_JSON,
...
def _import_local_stub(module_name: str):
    ...
    if module_path is None:
        return module
    try:
        if Path(module_path).resolve().is_relative_to(_REPO_ROOT):
            return module
    except AttributeError:  # pragma: no cover - Python < 3.9
        if str(Path(module_path).resolve()).startswith(str(_REPO_ROOT)):
            return module
    return None


_SHARED_DATA = _import_local_stub("shared_data")
_SHARED_YAML = _import_local_stub("shared_yaml")
...
if _SHARED_DATA and hasattr(_SHARED_DATA, "REPO_ROOT"):
    ...
    REPO_ROOT = _SD_ROOT
    PLANNING_INDEX_JSON = Path(getattr(_SHARED_DATA, "PLANNING_INDEX_JSON", _SD_ROOT / "data" / "planning_index.json"))
...
def export_markdown(output_path: Path | None = None):
    ...
    if result.is_success():
        ...
        prov_report_path = REPORTS_DIR / "roadmap_export_provenance.json"
