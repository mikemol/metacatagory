...
trimQuotes s = strip (primStringToList s)
  where
    mutual
      strip : List Char → String
      strip [] = ""
      strip ('"' ∷ cs) = stripEnd cs
      strip cs = primStringFromList cs

      stripEnd : List Char → String
      stripEnd [] = ""
      stripEnd ('"' ∷ []) = ""
      stripEnd ('"' ∷ cs) = primStringFromList cs
      stripEnd (c ∷ cs) = primStringFromList (c ∷ cs)
