...
agdaTargets =
  ...
  ∷ generatorToTarget "build/diagrams/agda-deps-full.dot" "Generate dependency graph" ([]) 
    ("mkdir -p build/diagrams" ∷ "$(AGDA) --dependency-graph=build/diagrams/agda-deps-full.dot $(AGDA_FLAGS) src/agda/Tests/Index.agda 2>&1 | (grep -E \"(Checking|Error)\" || true) | head -20" ∷ [])
